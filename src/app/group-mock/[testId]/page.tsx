...
export default function TestPage() {
  ...
  const [activeTestInstanceId, setActiveTestInstanceId] = useState<
    string | null
  >(null);
  ...
  useEffect(() => {
    ...
    const loadTest = async () => {
      try {
        ...
        setActiveTestInstanceId(instanceId);

        // Step 2 & 3: Get test details and saved progress
        const testData = await apiService.getScheduledTestDetails(instanceId);
        const savedProgress = await apiService.getSavedMarkProgress(instanceId);
  ...
  const saveProgress = useCallback(
    async (questionToSave: Question | undefined | null) => {
      // --- FIX: Use the activeTestInstanceId from state ---
      if (!questionToSave || !activeTestInstanceId) return;
    ...
    [activeTestInstanceId] // Dependency on the correct ID
  );
  ...
  const handleSubmitTest = useCallback(async () => {
    // The check now only depends on the instance ID
    if (!activeTestInstanceId) {
      alert("Cannot submit test. Session ID is missing.");
      return;
    }
    try {
      ...
      // Submit the test using the specific attempt's ID
      await apiService.submitScheduledTest(activeTestInstanceId);

      // --- CHANGE: Redirect using the activeTestInstanceId ---
      // This changes the results URL to be specific to this user's attempt.
      router.push(`/group-mock-results/${activeTestInstanceId}`);
    } catch (err) {
      alert("Failed to submit test. Please try again.");
      console.error(err);
    }
  }, [
    activeTestInstanceId, // The redirect ID is now the instance ID
    router,
    saveProgress,
    currentQuestion,
  ]);
